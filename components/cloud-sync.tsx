...
export default function CloudSync({ children }: Props) {
  ...
  const [ready, setReady] = useState(false); const [userEmail, setUserEmail] = useState(""); const [authMode, setAuthMode] = useState<"login" | "signup">("login"); const [email, setEmail] = useState(""); const [password, setPassword] = useState(""); const [busy, setBusy] = useState(false); const [message, setMessage] = useState(""); const lastUploaded = useRef(""); const syncing = useRef(false);
  ...
  const loadCloud = useCallback(async () => {
    ...
    const latestReport = reportRows?.[0]?.content; if (typeof latestReport === "string" && latestReport !== (localStorage.getItem(REPORT_KEY) ?? "")) { localStorage.setItem(REPORT_KEY, latestReport); window.location.reload(); }
  }, [supabase]);

  const uploadLocal = useCallback(async () => {
    if (!supabase || syncing.current) return; syncing.current = true;
    try {
      const { data: { user } } = await supabase.auth.getUser(); if (!user) return; const localTasks = safeTasks(); const signature = JSON.stringify(normalizedTasks(localTasks));
      ...
      if (report.trim() && localTasks.length) { const dates = localTasks.map((task) => task.date).sort(); const { error } = await supabase.from("work_reports").upsert({ user_id: user.id, period_start: dates[0], period_end: dates[dates.length - 1], content: report, updated_at: new Date().toISOString() }, { onConflict: "user_id,period_start,period_end" }); if (error) throw error; }
    } catch (error) { console.error("Worklog cloud sync failed", error); } finally { syncing.current = false; }
  }, [supabase]);

  useEffect(() => {
    if (!supabase) { setReady(true); return; } let mounted = true;
    const init = async () => {
